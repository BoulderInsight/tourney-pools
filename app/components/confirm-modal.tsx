    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-6">
      ...
      <div className="relative bg-white rounded-2xl shadow-card-lg w-full max-w-sm overflow-hidden animate-slide-up">
        <div className="p-6 text-center">
          {/* Icon */}
          <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${danger ? "bg-red-50" : "bg-masters-green/10"}`}>
            {danger ? (
              <svg className="w-7 h-7 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            ) : (
              <svg className="w-7 h-7 text-masters-green" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <button
            ...
            className={`flex-1 py-4 text-sm font-semibold transition-colors ${
              danger
                ...
                : "text-masters-green active:bg-masters-green/5"
            }`}
          >
            {confirmLabel}
          </button>
